    ...
    for epoch in range(args.num_epochs):
        ...
        if best_val_kapp < log_dict["val_bag_kap"][-1]:
            ...
            save_confusion_matrix(cm=train_ins_cm, path=("%s/cm/fold=%d_seed=%d-cm_train_ins.png") % (args.output_path, args.fold, args.seed),
                        title='train: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["train_ins_acc"][epoch], log_dict["train_ins_kap"][epoch], log_dict["train_ins_f1"][epoch]))
            save_confusion_matrix(cm=train_bag_cm, path=("%s/cm/fold=%d_seed=%d-cm_train_bag.png") % (args.output_path, args.fold, args.seed),
                        title='train: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["train_bag_acc"][epoch], log_dict["train_bag_kap"][epoch], log_dict["train_bag_f1"][epoch]))
            save_confusion_matrix(cm=val_ins_cm, path=("%s/cm/fold=%d_seed=%d-cm_val_ins.png") % (args.output_path, args.fold, args.seed),
                        title='val: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["val_ins_acc"][epoch], log_dict["val_ins_kap"][epoch], log_dict["val_ins_f1"][epoch]))
            save_confusion_matrix(cm=val_bag_cm, path=("%s/cm/fold=%d_seed=%d-cm_val_bag.png") % (args.output_path, args.fold, args.seed),
                        title='val: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["val_bag_acc"][epoch], log_dict["val_bag_kap"][epoch], log_dict["val_bag_f1"][epoch]))
            if args.is_test == True:
                save_confusion_matrix(cm=test_ins_cm, path=("%s/cm/fold=%d_seed=%d-cm_test_ins.png") % (args.output_path, args.fold, args.seed),
                            title='test: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["test_ins_acc"][epoch], log_dict["test_ins_kap"][epoch], log_dict["test_ins_f1"][epoch]))
                save_confusion_matrix(cm=test_bag_cm, path=("%s/cm/fold=%d_seed=%d-cm_test_bag.png") % (args.output_path, args.fold, args.seed),
                            title='test: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:%.4f' % (epoch+1, log_dict["test_bag_acc"][epoch], log_dict["test_bag_kap"][epoch], log_dict["test_bag_f1"][epoch]))
        else:
            cnt += 1
