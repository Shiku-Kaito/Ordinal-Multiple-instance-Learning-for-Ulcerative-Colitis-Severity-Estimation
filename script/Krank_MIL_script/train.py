...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function):
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            ...
            e_time = time()
            logging.info('[Epoch: %d/%d (%ds)] @Test Bag emsemble acc: %.4f, kapp: %.4f, macro-f1: %.4f,' %
                        (epoch+1, args.num_epochs, e_time-s_time, log_dict["test_emsemble_bag_acc"][-1], log_dict["test_emsemble_bag_kap"][-1], log_dict["test_emsemble_bag_f1"][-1]))
