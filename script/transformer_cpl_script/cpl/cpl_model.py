...
class Transformer_CPL(nn.Module):
    def __init__(
        self,
        num_classes=1000,
        embed_dim=768,
    ...
    def forward(self, x, len_list):
        ...
        assign_metric = self.metric_method(x[:, 0, :], proxies)
        proxies_metric = self.metric_method(proxies, proxies)


        # x1 = self.head1(x[:, :1, :]).squeeze()
    
        return {"atten_weight":attn_weights, "assign_metric":assign_metric, "proxies_metric":proxies_metric} #"bag": x1,
