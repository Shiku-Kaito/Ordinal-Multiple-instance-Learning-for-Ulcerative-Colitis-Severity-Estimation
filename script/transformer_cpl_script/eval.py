...
def eval_net(args, model, test_loader):
    ...
    with torch.no_grad():
        for iteration, data in enumerate(test_loader): #enumerate(tqdm(test_loader, leave=False)):
            ...
            bag_pred.extend(y["assign_metric"].argmax(-1).cpu().detach().numpy())
            # ins_pred.extend(y["ins"].argmax(1).cpu().detach().numpy())
            ins_gt.extend(ins_label.cpu().detach().numpy())

    bag_gt, bag_pred, ins_pred, ins_gt =np.array(bag_gt), np.array(bag_pred), np.array(ins_pred), np.array(ins_gt)
    ...
    result_dict["max_labels"].extend(bag_gt), result_dict["bag_pred"].extend(bag_pred)
    return result_dict