...
class Transfomer_Krank(nn.Module):
    def __init__(
        self,
        num_classes=1000,
        embed_dim=768,
    ...
    def forward(self, x, len_list):
        ...
        x2 = self.head2(x[:, :1, :]).squeeze()
        x3 = self.head3(x[:, :1, :]).squeeze()
        return {"y_0vs123": x1, "y_01vs23":x2, "y_012vs3":x3, "bag_feat": x[:, :1, :], "ins_feats":ins_feats, "atten_weight":attn_weights, "cls_token_feat": self.cls_token}
