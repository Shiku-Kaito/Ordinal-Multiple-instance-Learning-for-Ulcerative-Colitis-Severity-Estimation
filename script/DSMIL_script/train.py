...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function):
    loss_IC, loss_BC = loss_function["IC_loss"], loss_function["BC_loss"]
    fix_seed(args.seed)
    log_dict = {"train_bag_acc":[], "train_ins_acc":[], "train_bag_kap":[], "train_ins_kap":[], "train_bag_f1":[], "train_ins_f1":[], "train_mIoU":[], "train_loss":[], "train_loss1":[], "train_loss2":[],
                "val_bag_acc":[], "val_ins_acc":[], "val_bag_kap":[], "val_ins_kap":[], "val_bag_f1":[], "val_ins_f1":[], "val_mIoU":[], "val_loss":[], "val_loss1":[], "val_loss2":[],
                "test_bag_acc":[], "test_ins_acc":[], "test_bag_kap":[], "test_ins_kap":[], "test_bag_f1":[], "test_ins_f1":[], "test_mIoU":[], "test_loss":[], "test_loss1":[], "test_loss2":[]}
    ...
    for epoch in range(args.num_epochs):
        ...
        s_time = time()
        model.train()
        ins_gt, bag_gt, ins_pred, bag_pred, losses, losses1, losses2 = [], [], [], [], [], [], []
        for iteration, data in enumerate(train_loader): #enumerate(tqdm(train_loader, leave=False)):
            bags, ins_label, bag_label_reg, bag_label = data["bags"], data["ins_label"], data["max_label"], data["max_label_CL"]
            bags, ins_label, bag_label_reg, bag_label = bags.to(args.device), ins_label.to(args.device), bag_label_reg.to(args.device), bag_label.to(args.device)

            y = model(x=bags, len_list=data["len_list"])
            loss1 = loss_IC(y["IC"].view(-1), bag_label_reg)
            loss2 = loss_BC(y["BC"], bag_label)
            ...
            bag_pred.extend(y["BC"].argmax(1).cpu().detach().numpy())
            losses.append(loss.item()), losses1.append(loss1.item()), losses2.append(loss2.item())

        ins_gt, bag_gt, bag_pred = np.array(ins_gt), np.array(bag_gt), np.array(bag_pred)
        ...
        log_dict["train_loss"].append(np.array(losses).mean()), log_dict["train_loss1"].append(np.array(losses1).mean()), log_dict["train_loss2"].append(np.array(losses2).mean())

        e_time = time()
        logging.info('[Epoch: %d/%d (%ds)] train loss: %.4f, @bag acc: %.4f, kapp: %.4f, macro-f1: %.4f' %
        ...
        log_dict["val_loss"].append(np.array(losses).mean()), log_dict["val_loss1"].append(np.array(losses1).mean()), log_dict["val_loss2"].append(np.array(losses2).mean())

        e_time = time()
        logging.info('[Epoch: %d/%d (%ds)] val loss: %.4f, @bag acc: %.4f, kapp: %.4f, macro-f1: %.4f' %
        ...
        if args.is_test == True:
            ...
            e_time = time()
            logging.info('[Epoch: %d/%d (%ds)]  @bag acc: %.4f, kapp: %.4f, macro-f1: %.4f' %
