    ...
    for epoch in range(args.num_epochs):
        ...
        if best_val_kapp < log_dict["val_bag_kap"][-1]:
            ...
            if args.c_path:
                ...
                torch.save(model.state_dict(), ("%s/model/fold=%d_seed=%d-pretrain_best_model.pkl") % (args.output_path, args.fold, args.seed))
                save_confusion_matrix(cm=train_bag_cm, path=("%s/cm/fold=%d_seed=%d-pretrain_cm_train_bag.png") % (args.output_path, args.fold, args.seed),
                            title='train: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:n%.4f' % (epoch+1, log_dict["train_bag_acc"][epoch], log_dict["train_bag_kap"][epoch], log_dict["train_bag_f1"][epoch]))
                save_confusion_matrix(cm=val_bag_cm, path=("%s/cm/fold=%d_seed=%d-pretrain_cm_val_bag.png") % (args.output_path, args.fold, args.seed),
                            title='val: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:n%.4f' % (epoch+1, log_dict["val_bag_acc"][epoch], log_dict["val_bag_kap"][epoch], log_dict["val_bag_f1"][epoch]))
                if args.is_test == True:
                    save_confusion_matrix(cm=test_bag_cm, path=("%s/cm/fold=%d_seed=%d-pretrain_cm_test_bag.png") % (args.output_path, args.fold, args.seed),
                                title='test: epoch: %d, acc: %.4f, kapp: %.4f, macro-f1:n%.4f' % (epoch+1, log_dict["test_bag_acc"][epoch], log_dict["test_bag_kap"][epoch], log_dict["test_bag_f1"][epoch]))
        ...
        if args.c_path:
            ...
            make_loss_graph(args,log_dict['train_loss'], log_dict['val_loss'], "%s/loss_graph/fold=%d_seed=%d_pretrain_loss-graph.png" % (args.output_path, args.fold, args.seed))
            make_bag_acc_graph(args, log_dict['train_bag_acc'], log_dict['val_bag_acc'], log_dict['test_bag_acc'], "%s/acc_graph/fold=%d_seed=%d_pretrain_bag-acc-graph.png" % (args.output_path, args.fold, args.seed))
            make_ins_acc_graph(args, log_dict['train_ins_acc'], log_dict['val_ins_acc'], log_dict['test_ins_acc'], "%s/acc_graph/fold=%d_seed=%d_pretrain_ins-acc-graph.png" % (args.output_path, args.fold, args.seed))
            make_bag_acc_graph(args, log_dict['train_bag_kap'], log_dict['val_bag_kap'], log_dict['test_bag_kap'], "%s/acc_graph/fold=%d_seed=%d_pretrain_bag-kap-graph.png" % (args.output_path, args.fold, args.seed))
            make_ins_acc_graph(args, log_dict['train_ins_kap'], log_dict['val_ins_kap'], log_dict['test_ins_kap'], "%s/acc_graph/fold=%d_seed=%d_pretrain_ins-kap-graph.png" % (args.output_path, args.fold, args.seed))
            make_bag_acc_graph(args, log_dict['train_bag_f1'], log_dict['val_bag_f1'], log_dict['test_bag_f1'], "%s/acc_graph/fold=%d_seed=%d_pretrain_bag-macrof1-graph.png" % (args.output_path, args.fold, args.seed))
            make_ins_acc_graph(args, log_dict['train_ins_f1'], log_dict['val_ins_f1'], log_dict['test_ins_f1'], "%s/acc_graph/fold=%d_seed=%d_pretrain_ins-macrof1-graph.png" % (args.output_path, args.fold, args.seed))
            np.save("%s/log_dict/fold=%d_seed=%d_pretrain_log" % (args.output_path, args.fold, args.seed) , log_dict)     
    return