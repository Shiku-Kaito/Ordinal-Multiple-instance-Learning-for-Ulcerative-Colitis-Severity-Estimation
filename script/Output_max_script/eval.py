...
def eval_net(args, model, test_loader):
    ...
    with torch.no_grad():
        for iteration, data in enumerate(test_loader): #enumerate(tqdm(test_loader, leave=False)):
            ...
            ins_pred.extend(y["ins"].cpu().detach().numpy()), bag_pred.extend(y["bag"].cpu().detach().numpy())
            ins_feat.extend(y["ins_feats"].cpu().detach().numpy())
            bag_feat.extend(y["bag_feat"].cpu().detach().numpy())

            bag_path.extend(data["bag_path"])
            ins_score.extend(y["ins_score"])

    ins_gt, bag_gt, ins_pred, bag_pred = np.array(ins_gt), np.array(bag_gt), np.array(ins_pred), np.array(bag_pred)
    bag_pred  = ORlabel_translation(bag_pred)
    ...
    result_dict["bag_pred"] = bag_pred
    result_dict["bag_feat"] = bag_feat
    result_dict["max_labels"], result_dict["bag_pred"] = [], []
    result_dict["max_labels"].extend(bag_gt)
    result_dict["ins_feat"] = np.array(ins_feat)
    result_dict["ins_labels"] = ins_gt
    result_dict["bag_path"] = bag_path
    result_dict["ins_score"] = ins_score
